import './style.css';
import { getDatabase, euclideanDistance, getState, importData } from './database';
...
import { vectorSearchFullScan, vectorSearchIndexRange } from './search';
...
async function run() {
    ...
    console.log(`Initial database state: ${itemCount} items, ${vectorCount} vectors`);
    if (itemCount > 0) {
...
function textToHtml(item: { id: string, title: string, body: string }, nr: number) {
    if (!item || !item.body) {
